...
export function DeleteButtonTask({ onDelete, onDeleteSuccess }) {
  ...
  const handleDelete = async (e) => {
    ...
    try {
      await onDelete();
      setShowModal(false);
      if (onDeleteSuccess) onDeleteSuccess(); // dispara callback para remover card
    } catch (error) {
  ...
  return (
    <div>
      ...
      <button
        ...
        className="flex items-center justify-center gap-2 w-full px-6 py-3 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-400 transition-colors cursor-pointer"
      >
          ...
          >
            ...
            <div className="flex flex-col space-y-3">
              <button
                ...
                className={`px-4 py-2 rounded-md cursor-pointer text-white text-sm font-medium tracking-wide ${
                  isDeleting
